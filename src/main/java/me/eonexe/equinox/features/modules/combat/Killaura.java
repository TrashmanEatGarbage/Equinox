...
class Killaura
    ...
    void onUpdateWalkingPlayerEvent ( UpdateWalkingPlayerEvent event ) {
        if ( event.getStage ( ) == 0 && this.rotate.getValue ( ) ) {
            if ( this.stay.getValue ( ) && target != null ) {
                Equinox.rotationManager.lookAtEntity ( target );
            }
            this.doKillaura ( );
        }
    }
    ...
    void doKillaura ( ) {
        ...
        int wait = ! this.delay.getValue ( ) || EntityUtil2.holding32k ( Killaura.mc.player ) && ! this.teekaydelay.getValue ( ) ? 0 : (int) ( (float) DamageUtil.getCooldownByWeapon ( Killaura.mc.player ) * ( this.tps.getValue ( ) ? Equinox.serverManager.getTpsFactor ( ) : 1.0f ) );
        ...
        if ( this.teleport.getValue ( ) ) {
            Equinox.positionManager.setPositionPacket ( Killaura.target.posX , EntityUtil.canEntityFeetBeSeen ( target ) ? Killaura.target.posY : Killaura.target.posY + (double) target.getEyeHeight ( ) , Killaura.target.posZ , true , true , ! this.lagBack.getValue ( ) );
        }
        if ( EntityUtil2.holding32k ( Killaura.mc.player ) && ! this.teekaydelay.getValue ( ) ) {

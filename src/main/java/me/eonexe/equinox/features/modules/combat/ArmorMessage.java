...
class ArmorMessage
    ...
    void onUpdate ( UpdateWalkingPlayerEvent event ) {
        for (EntityPlayer player : ArmorMessage.mc.world.playerEntities) {
            if ( player.isDead || ! Equinox.friendManager.isFriend ( player.getName ( ) ) ) continue;
            for (ItemStack stack : player.inventory.armorInventory) {
                ...
                if ( percent <= this.armorThreshhold.getValue ( ) && ! this.entityArmorArraylist.containsKey ( player ) ) {
                    if ( player == ArmorMessage.mc.player && this.notifySelf.getValue ( ) ) {
                        Command.sendClientMessage ( player.getName ( ) + " watchout your " + this.getArmorPieceName ( stack ) + " low dura!", 69420 );
                    } else {

...
import me.eonexe.equinox.event.events.ProcessRightClickBlockEvent;
import me.eonexe.equinox.features.modules.Module;
import me.eonexe.equinox.features.modules.client.ServerModule;
import me.eonexe.equinox.features.setting.Bind;
import me.eonexe.equinox.features.setting.EnumConverter;
import me.eonexe.equinox.features.setting.Setting;
import me.eonexe.equinox.mixin.mixins.IContainer;
import me.eonexe.equinox.mixin.mixins.ISPacketSetSlot;
...
public class PhobosOffhand
    ...
    public void onItemFinish(ItemStack stack, EntityLivingBase base) {
        if (this.noOffhandGC.getValue().booleanValue() && base.equals(PhobosOffhand.mc.player) && stack.getItem() == PhobosOffhand.mc.player.getHeldItemOffhand().getItem()) {
    ...
    public void onUpdateWalkingPlayer(ProcessRightClickBlockEvent event) {
        if (this.noOffhandGC.getValue().booleanValue() && event.hand == EnumHand.MAIN_HAND && event.stack.getItem() == Items.END_CRYSTAL && PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && PhobosOffhand.mc.objectMouseOver != null && event.pos == PhobosOffhand.mc.objectMouseOver.getBlockPos()) {
            event.setCanceled(true);
            PhobosOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
            PhobosOffhand.mc.playerController.processRightClick(PhobosOffhand.mc.player, PhobosOffhand.mc.world, EnumHand.OFF_HAND);
        }
    }
    ...
    public void onUpdate() {
        if (this.noOffhandGC.getValue().booleanValue() && this.retardOGC.getValue().booleanValue()) {
            if (this.timer.passedMs(this.timeout.getValue().intValue())) {
                if (PhobosOffhand.mc.player != null && PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && PhobosOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && Mouse.isButtonDown(1)) {
                    PhobosOffhand.mc.player.setActiveHand(EnumHand.OFF_HAND);
                    PhobosOffhand.mc.gameSettings.keyBindUseItem.pressed = Mouse.isButtonDown(1);
                }
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (this.noOffhandGC.getValue().booleanValue() && !PhobosOffhand.fullNullCheck() && PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && PhobosOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL && PhobosOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (ServerModule.getInstance().isConnected() && event.getPacket() instanceof SPacketSetSlot && (packet = event.getPacket()).getSlot() == -1 && packet.getWindowId() != -1) {
            ((IContainer) PhobosOffhand.mc.player.openContainer).setTransactionID((short) packet.getWindowId());
    ...
    public void doOffhand() {
        ...
        if (this.type.getValue() == Type.NEW) {
            if (PhobosOffhand.mc.currentScreen instanceof GuiContainer && !this.guis.getValue().booleanValue() && !(PhobosOffhand.mc.currentScreen instanceof GuiInventory)) {
                return;
            }
            if (this.gapSwap.getValue().booleanValue()) {
                if ((this.getSlot(Mode.GAPPLES) != -1 || PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE) && PhobosOffhand.mc.player.getHeldItemMainhand().getItem() != Items.GOLDEN_APPLE && PhobosOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
                    this.setMode(Mode.GAPPLES);
            ...
            if (!this.shouldTotem()) {
                if (PhobosOffhand.mc.player.getHeldItemOffhand() == ItemStack.EMPTY || !this.isItemInOffhand()) {
                    int slot;
                    int n = slot = this.getSlot(this.mode) < 9 ? this.getSlot(this.mode) + 36 : this.getSlot(this.mode);
                    if (this.getSlot(this.mode) != -1) {
                        if (this.oldSlot != -1) {
                            PhobosOffhand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, PhobosOffhand.mc.player);
                            PhobosOffhand.mc.playerController.windowClick(0, this.oldSlot, 0, ClickType.PICKUP, PhobosOffhand.mc.player);
                        }
                        this.oldSlot = slot;
                        PhobosOffhand.mc.playerController.windowClick(0, slot, 0, ClickType.PICKUP, PhobosOffhand.mc.player);
                        PhobosOffhand.mc.playerController.windowClick(0, 45, 0, ClickType.PICKUP, PhobosOffhand.mc.player);
                        PhobosOffhand.mc.playerController.windowClick(0, slot, 0, ClickType.PICKUP, PhobosOffhand.mc.player);
                    }
                }
            } else if (!(this.eatingApple || PhobosOffhand.mc.player.getHeldItemOffhand() != ItemStack.EMPTY && PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)) {
                int slot;
            ...
            this.holdingCrystal = PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
            this.holdingTotem = PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING;
            this.holdingGapple = PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE;
            this.holdingObby = InventoryUtil.isBlock(PhobosOffhand.mc.player.getHeldItemOffhand().getItem(), BlockObsidian.class);
            this.holdingWeb = InventoryUtil.isBlock(PhobosOffhand.mc.player.getHeldItemOffhand().getItem(), BlockWeb.class);
            this.totems = PhobosOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
            if (this.holdingTotem) {
                this.totems += PhobosOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.TOTEM_OF_UNDYING).mapToInt(ItemStack::getCount).sum();
            }
            this.crystals = PhobosOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.END_CRYSTAL).mapToInt(ItemStack::getCount).sum();
            if (this.holdingCrystal) {
                this.crystals += PhobosOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.END_CRYSTAL).mapToInt(ItemStack::getCount).sum();
            }
            this.gapples = PhobosOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> itemStack.getItem() == Items.GOLDEN_APPLE).mapToInt(ItemStack::getCount).sum();
            if (this.holdingGapple) {
                this.gapples += PhobosOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> itemStack.getItem() == Items.GOLDEN_APPLE).mapToInt(ItemStack::getCount).sum();
            }
            if (this.currentMode == Mode2.WEBS || this.currentMode == Mode2.OBSIDIAN) {
                this.obby = PhobosOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockObsidian.class)).mapToInt(ItemStack::getCount).sum();
                if (this.holdingObby) {
                    this.obby += PhobosOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockObsidian.class)).mapToInt(ItemStack::getCount).sum();
                }
                this.webs = PhobosOffhand.mc.player.inventory.mainInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockWeb.class)).mapToInt(ItemStack::getCount).sum();
                if (this.holdingWeb) {
                    this.webs += PhobosOffhand.mc.player.inventory.offHandInventory.stream().filter(itemStack -> InventoryUtil.isBlock(itemStack.getItem(), BlockWeb.class)).mapToInt(ItemStack::getCount).sum();
                }
            }
            this.doSwitch(false);
        }
    ...
    public void doSwitch(boolean b) {
        if (this.autoGapple.getValue().booleanValue()) {
            if (PhobosOffhand.mc.gameSettings.keyBindUseItem.isKeyDown()) {
                if (PhobosOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword && (!this.onlyWTotem.getValue().booleanValue() || PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING)) {
                    this.setMode(Mode.GAPPLES);
        ...
        if (this.currentMode == Mode2.GAPPLES && ((!EntityUtil.isSafe(PhobosOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.gappleHealth.getValue().floatValue() || EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.gappleHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.CRYSTALS && ((!EntityUtil.isSafe(PhobosOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.crystalHealth.getValue().floatValue() || EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.crystalHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.OBSIDIAN && ((!EntityUtil.isSafe(PhobosOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.obsidianHealth.getValue().floatValue() || EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.obsidianHoleHealth.getValue().floatValue()) || this.currentMode == Mode2.WEBS && ((!EntityUtil.isSafe(PhobosOffhand.mc.player) || this.bedPlaceable()) && EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.webHealth.getValue().floatValue() || EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) <= this.webHoleHealth.getValue().floatValue())) {
            if (this.returnToCrystal.getValue().booleanValue() && this.currentMode == Mode2.CRYSTALS) {
                this.switchedForHealthReason = true;
            }
            this.setMode(Mode2.TOTEMS);
        }
        if (this.switchedForHealthReason && (EntityUtil.isSafe(PhobosOffhand.mc.player) && !this.bedPlaceable() && EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) > this.crystalHoleHealth.getValue().floatValue() || EntityUtil.getHealth(PhobosOffhand.mc.player, this.absorption.getValue()) > this.crystalHealth.getValue().floatValue())) {
            this.setMode(Mode2.CRYSTALS);
        ...
        Item currentOffhandItem = PhobosOffhand.mc.player.getHeldItemOffhand().getItem();
        switch (this.currentMode) {
            case TOTEMS: {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && PhobosOffhand.mc.world.playerEntities.stream().noneMatch(e -> e != PhobosOffhand.mc.player && !Equinox.friendManager.isFriend(e) && PhobosOffhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }

    private boolean isItemInOffhand() {
        switch (this.mode) {
            case GAPPLES: {
                return PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE;
            }
            case CRYSTALS: {
                return PhobosOffhand.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
            }
            case OBSIDIAN: {
                return PhobosOffhand.mc.player.getHeldItemOffhand().getItem() instanceof ItemBlock && ((ItemBlock) PhobosOffhand.mc.player.getHeldItemOffhand().getItem()).block == Blocks.OBSIDIAN;
            }
            case WEBS: {
                return PhobosOffhand.mc.player.getHeldItemOffhand().getItem() instanceof ItemBlock && ((ItemBlock) PhobosOffhand.mc.player.getHeldItemOffhand().getItem()).block == Blocks.WEB;
            }
        }
        return false;
    }
    ...
    private boolean isHeldInMainHand() {
        switch (this.mode) {
            case GAPPLES: {
                return PhobosOffhand.mc.player.getHeldItemMainhand().getItem() == Items.GOLDEN_APPLE;
            }
            case CRYSTALS: {
                return PhobosOffhand.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL;
            }
            case OBSIDIAN: {
                return PhobosOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && ((ItemBlock) PhobosOffhand.mc.player.getHeldItemMainhand().getItem()).block == Blocks.OBSIDIAN;
            }
            case WEBS: {
                return PhobosOffhand.mc.player.getHeldItemMainhand().getItem() instanceof ItemBlock && ((ItemBlock) PhobosOffhand.mc.player.getHeldItemMainhand().getItem()).block == Blocks.WEB;
            }
        }
        return false;
    }
    ...
    private boolean shouldTotem() {
        ...
        if (this.holeCheck.getValue().booleanValue() && EntityUtil.isInHole(PhobosOffhand.mc.player) && !this.bedPlaceable()) {
            return PhobosOffhand.mc.player.getHealth() + PhobosOffhand.mc.player.getAbsorptionAmount() <= this.getHoleHealth() || PhobosOffhand.mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.ELYTRA || PhobosOffhand.mc.player.fallDistance >= 3.0f || this.noNearbyPlayers() || this.crystalCheck.getValue() != false && this.isCrystalsAABBEmpty();
        }
        return PhobosOffhand.mc.player.getHealth() + PhobosOffhand.mc.player.getAbsorptionAmount() <= this.getHealth() || PhobosOffhand.mc.player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == Items.ELYTRA || PhobosOffhand.mc.player.fallDistance >= 3.0f || this.noNearbyPlayers() || this.crystalCheck.getValue() != false && this.isCrystalsAABBEmpty();
    }

    private boolean isNotEmpty(BlockPos pos) {
        return PhobosOffhand.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(pos)).stream().anyMatch(e -> e instanceof EntityEnderCrystal);
    }
    ...
    private boolean isCrystalsAABBEmpty() {
        return this.isNotEmpty(PhobosOffhand.mc.player.getPosition().add(1, 0, 0)) || this.isNotEmpty(PhobosOffhand.mc.player.getPosition().add(-1, 0, 0)) || this.isNotEmpty(PhobosOffhand.mc.player.getPosition().add(0, 0, 1)) || this.isNotEmpty(PhobosOffhand.mc.player.getPosition().add(0, 0, -1)) || this.isNotEmpty(PhobosOffhand.mc.player.getPosition());
    }
    ...
    int getStackSize() {
        ...
        if (this.shouldTotem()) {
            for (int i = 45; i > 0; --i) {
                if (PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem() != Items.TOTEM_OF_UNDYING) continue;
                size += PhobosOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else if (this.mode == Mode.OBSIDIAN) {
            for (int i = 45; i > 0; --i) {
                if (!(PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock) PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem()).block != Blocks.OBSIDIAN)
                    continue;
                size += PhobosOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else if (this.mode == Mode.WEBS) {
            for (int i = 45; i > 0; --i) {
                if (!(PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) || ((ItemBlock) PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem()).block != Blocks.WEB)
                    continue;
                size += PhobosOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        } else {
            for (int i = 45; i > 0; --i) {
                if (PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem() != (this.mode == Mode.CRYSTALS ? Items.END_CRYSTAL : Items.GOLDEN_APPLE))
                    continue;
                size += PhobosOffhand.mc.player.inventory.getStackInSlot(i).getCount();
            }
        }
    ...
    int getSlot(Mode m) {
        ...
        if (m == Mode.OBSIDIAN) {
            ...
            for (int i = 45; i > 0; --i) {
                if (PhobosOffhand.mc.player.inventory.getStackInSlot(i).getItem() != (m == Mode.CRYSTALS ? Items.END_CRYSTAL : Items.GOLDEN_APPLE))
                    continue;
                slot = i;
                break;
            }
    ...
    private boolean bedPlaceable() {
        ...
        if (PhobosOffhand.mc.world.getBlockState(PhobosOffhand.mc.player.getPosition()).getBlock() != Blocks.BED && PhobosOffhand.mc.world.getBlockState(PhobosOffhand.mc.player.getPosition()).getBlock() != Blocks.AIR) {
            return false;
        }
        for (EnumFacing facing : EnumFacing.values()) {
            if (facing == EnumFacing.UP || facing == EnumFacing.DOWN || PhobosOffhand.mc.world.getBlockState(PhobosOffhand.mc.player.getPosition().offset(facing)).getBlock() != Blocks.BED && PhobosOffhand.mc.world.getBlockState(PhobosOffhand.mc.player.getPosition().offset(facing)).getBlock() != Blocks.AIR)
                continue;
            return true;
        }
        return false;

...
import me.eonexe.equinox.features.modules.misc.AutoGG;
...
public class AutoCrystal
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            if (AutoGG.getINSTANCE().isOn()) {
                AutoGG autoGG = (AutoGG) Equinox.moduleManager.getModuleByName("AutoGG");
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            if (AutoCrystal.mc.player == null || AutoCrystal.mc.player.isDead || entity.isDead || entity == AutoCrystal.mc.player || Equinox.friendManager.isFriend(entity.getName()) || entity.getDistance(AutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;

...
public class InventoryUtil3
        implements Util {
    public static void switchToHotbarSlot(int slot, boolean silent) {
        if (InventoryUtil3.mc.player.inventory.currentItem == slot || slot < 0) {
            return;
        }
        if (silent) {
            InventoryUtil3.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
            InventoryUtil3.mc.playerController.updateController();
        } else {
            InventoryUtil3.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
            InventoryUtil3.mc.player.inventory.currentItem = slot;
            InventoryUtil3.mc.playerController.updateController();
        }
    }

    public static void switchToHotbarSlot(Class clazz, boolean silent) {
        int slot = InventoryUtil3.findHotbarBlock(clazz);
        if (slot > -1) {
            InventoryUtil3.switchToHotbarSlot(slot, silent);
        }
    }
    ...
    public static int findHotbarBlock(Class clazz) {
        for (int i = 0; i < 9; ++i) {
            Block block;
            ItemStack stack = InventoryUtil3.mc.player.inventory.getStackInSlot(i);
    ...
    public static int getItemHotbar(Item input) {
        for (int i = 0; i < 9; ++i) {
            Item item = InventoryUtil3.mc.player.inventory.getStackInSlot(i).getItem();
    ...
    public static int findStackInventory(Item input) {
        return InventoryUtil3.findStackInventory(input, false);
    }
    ...
    public static int findItemInventorySlot(Item item, boolean offHand) {
        ...
        slot.set(-1);
        for (Map.Entry<Integer, ItemStack> entry : InventoryUtil3.getInventoryAndHotbarSlots().entrySet()) {
    ...
    public static List<Integer> findEmptySlots(boolean withXCarry) {
        ...
        if (withXCarry) {
            for (int i = 1; i < 5; ++i) {
                Slot craftingSlot = InventoryUtil3.mc.player.inventoryContainer.inventorySlots.get(i);
    ...
    public static int findInventoryBlock(Class clazz, boolean offHand) {
        ...
        slot.set(-1);
        for (Map.Entry<Integer, ItemStack> entry : InventoryUtil3.getInventoryAndHotbarSlots().entrySet()) {
            if (!InventoryUtil3.isBlock(entry.getValue().getItem(), clazz) || entry.getKey() == 45 && !offHand) continue;
            slot.set(entry.getKey());
            return slot.get();
        }
        return slot.get();
    ...
    public static Map<Integer, ItemStack> getInventoryAndHotbarSlots() {
        if (InventoryUtil3.mc.currentScreen instanceof GuiCrafting) {
            return InventoryUtil3.fuckYou3arthqu4kev2(10, 45);
        }
        return InventoryUtil3.getInventorySlots(9, 44);
    }
    ...
    private static Map<Integer, ItemStack> fuckYou3arthqu4kev2(int currentI, int last) {
        HashMap<Integer, ItemStack> fullInventorySlots = new HashMap<Integer, ItemStack>();
        for (int current = currentI; current <= last; ++current) {
            fullInventorySlots.put(current, InventoryUtil3.mc.player.openContainer.getInventory().get(current));
        }
        return fullInventorySlots;
    }
        ...
        switch (mode) {
            case NORMAL: {
                if (!back && !switchedItem) {
                    InventoryUtil3.switchToHotbarSlot(InventoryUtil3.findHotbarBlock(clazz), false);
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    InventoryUtil.switchToHotbarSlot(InventoryUtil3.findHotbarBlock(clazz), true);
                    switchedItemSwitched[0] = true;
                } else if (back && switchedItem) {
                    switchedItemSwitched[0] = false;
                    Equinox.inventoryManager.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
            case NONE: {
                switchedItemSwitched[1] = back || InventoryUtil3.mc.player.inventory.currentItem == InventoryUtil3.findHotbarBlock(clazz);
            }
        }
    ...
    public static boolean[] switchItemToItem(boolean back, int lastHotbarSlot, boolean switchedItem, Switch mode, Item item) {
        ...
        switch (mode) {
            case NORMAL: {
                if (!back && !switchedItem) {
                    InventoryUtil3.switchToHotbarSlot(InventoryUtil3.getItemHotbar(item), false);
                    switchedItemSwitched[0] = true;
                } else if (back && switchedItem) {
                    InventoryUtil3.switchToHotbarSlot(lastHotbarSlot, false);
                    switchedItemSwitched[0] = false;
                }
                switchedItemSwitched[1] = true;
                break;

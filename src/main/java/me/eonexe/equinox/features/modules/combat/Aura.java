...
public class Aura extends Module {
    ...
    private Setting<Boolean> attackPlayers = this.register(new Setting<>("Players", true));
    private Setting<Boolean> attackMobs = this.register(new Setting<>("Mobs", false));
    private Setting<Boolean> attackAnimals = this.register(new Setting<>("Animals", false));
    private Setting<Double> hitRange = this.register(new Setting<>("Hit Range", 5.5d));
    private Setting<Boolean> ignoreWalls = this.register(new Setting<>("Ignore Walls", true));
    private Setting<WaitMode> waitMode = this.register(new Setting<>("Mode", WaitMode.DYNAMIC));
    private Setting<Integer> waitTick = this.register(new Setting<>("Tick Delay", 3, 0, 3));
    private Setting<Boolean> switchTo32k = this.register(new Setting<>("32k Switch", true));
    private Setting<Boolean> onlyUse32k = this.register(new Setting<>("32k Only", false));

    private int waitCounter;

    public Aura() {
        super("Aura", "Hits entityies around you", Category.COMBAT, true, false, false);
    }
    ...
    public void onUpdate() {
        ...
        while (entityIterator.hasNext()) {
            ...
            if (attackPlayers.getValue() && target instanceof EntityPlayer && !Equinox.friendManager.isFriend(target.getName())) {
                attack(target);
                return;
            } else {

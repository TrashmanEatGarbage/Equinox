...
public class BedAura
    ...
    public void onDisable() {
        if (this.sslot.getValue().booleanValue()) {
            BedAura.mc.player.connection.sendPacket(new CPacketHeldItemChange(BedAura.mc.player.inventory.currentItem));
        }
    }
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (BedAura.fullNullCheck() || BedAura.mc.player.dimension != -1 && BedAura.mc.player.dimension != 1) {
            return;
        }
        if (event.getStage() == 0) {
            this.doBedBomb();
            if (this.shouldCraft && BedAura.mc.currentScreen instanceof GuiCrafting) {
                ...
                if (this.craftStage > 1 && !this.one) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 1, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.one = true;
                } else if (this.craftStage > 1 + this.craftDelay.getValue() && !this.two) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 2, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.two = true;
                } else if (this.craftStage > 1 + this.craftDelay.getValue() * 2 && !this.three) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 3, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woolSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.three = true;
                } else if (this.craftStage > 1 + this.craftDelay.getValue() * 3 && !this.four) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 4, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.four = true;
                } else if (this.craftStage > 1 + this.craftDelay.getValue() * 4 && !this.five) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 5, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.five = true;
                } else if (this.craftStage > 1 + this.craftDelay.getValue() * 5 && !this.six) {
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 6, 1, ClickType.PICKUP, BedAura.mc.player);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, woodSlot, 0, ClickType.PICKUP, BedAura.mc.player);
                    this.recheckBedSlots(woolSlot, woodSlot);
                    BedAura.mc.playerController.windowClick(((GuiContainer) BedAura.mc.currentScreen).inventorySlots.windowId, 0, 0, ClickType.QUICK_MOVE, BedAura.mc.player);
                    this.one = false;
                    this.two = false;
                    this.three = false;
                    this.four = false;
        ...
        if (this.explode.getValue().booleanValue() && this.breakTimer.passedMs(this.breakDelay.getValue().intValue())) {
            if (this.breakMode.getValue() == BreakLogic.CALC) {
                ...
                for (TileEntity entityBed : BedAura.mc.world.loadedTileEntityList) {
                    RayTraceResult result;
                    if (!(entityBed instanceof TileEntityBed) || BedAura.mc.player.getDistanceSq(entityBed.getPos()) > MathUtil.square(this.breakRange.getValue().floatValue()))
                        continue;
    ...
    private void mapBeds() {
        ...
        if (this.removeTiles.getValue().booleanValue()) {
            ...
            for (BedData data : removedBlocks) {
                float selfDamage;
                BlockPos pos;
                if (!data.isHeadPiece() || !(BedAura.mc.player.getDistanceSq(pos = data.getPos()) <= MathUtil.square(this.breakRange.getValue().floatValue())) || !((double) (selfDamage = DamageUtil.calculateDamage(pos, BedAura.mc.player)) + 1.0 < (double) EntityUtil.getHealth(BedAura.mc.player)) && DamageUtil.canTakeDamage(this.suicide.getValue()))
                    continue;
            ...
            for (TileEntity tile : BedAura.mc.world.loadedTileEntityList) {
                ...
                BlockPos pos;
                TileEntityBed bed;
                if (!(tile instanceof TileEntityBed) || !(bed = (TileEntityBed) tile).isHeadPiece() || !(BedAura.mc.player.getDistanceSq(pos = bed.getPos()) <= MathUtil.square(this.breakRange.getValue().floatValue())) || !((double) (selfDamage = DamageUtil.calculateDamage(pos, BedAura.mc.player)) + 1.0 < (double) EntityUtil.getHealth(BedAura.mc.player)) && DamageUtil.canTakeDamage(this.suicide.getValue()))
                    continue;
                for (EntityPlayer player : BedAura.mc.world.playerEntities) {
                    float damage;
                    if (!(player.getDistanceSq(pos) < MathUtil.square(this.range.getValue().floatValue())) || !EntityUtil.isValid(player, this.range.getValue().floatValue() + this.breakRange.getValue().floatValue()) || !((damage = DamageUtil.calculateDamage(pos, player)) > selfDamage || damage > this.minDamage.getValue().floatValue() && !DamageUtil.canTakeDamage(this.suicide.getValue())) && !(damage > EntityUtil.getHealth(player)) || !(damage > maxDamage))
    ...
    private void placeBed(BlockPos pos, boolean firstCheck) {
        ...
        float damage = DamageUtil.calculateDamage(pos, BedAura.mc.player);
        ...
        float[] rotation = RotationUtils.simpleFacing(this.finalFacing);
        ...
        this.shouldRotate.set(true);
        Equinox.rotationManager.setPlayerRotations(rotation[0], rotation[1]);
    }
    ...
    public void craftTable() {
        ...
        if (woodSlot != -1) {
            ...
            if (table != -1) {
                ...
                BedAura.mc.playerController.windowClick(0, this.tableSlot.getValue().intValue(), 0, ClickType.PICKUP, BedAura.mc.player);
                BedAura.mc.playerController.windowClick(0, table, 0, ClickType.PICKUP, BedAura.mc.player);
            }
        }
    }

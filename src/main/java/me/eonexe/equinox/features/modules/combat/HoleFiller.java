...
import me.eonexe.equinox.features.modules.client.PingBypass;
import me.eonexe.equinox.features.modules.player.BlockTweaks;
import me.eonexe.equinox.features.modules.player.Freecam;
...
class HoleFiller
    ...
    void onDisable ( ) {
        if ( this.offhand.getValue ( ) ) {
            Equinox.moduleManager.getModuleByClass ( Offhand.class ).setMode ( this.offhandMode );
            Equinox.moduleManager.getModuleByClass ( Offhand.class ).setMode ( this.offhandMode2 );
        }
    ...
    void doHoleFill ( ) {
        ...
        if ( this.midSafeHoles.getValue ( ) ) {
            object = Equinox.holeManager.getMidSafety ( );
            synchronized (object) {
            }
        }
        object = Equinox.holeManager.getHoles ( );
        synchronized (object) {
            targets = new ArrayList <> ( Equinox.holeManager.getHoles ( ) );
        }
    ...
    boolean check ( ) {
        ...
        this.switchItem ( true );
        if ( ! this.freecam.getValue ( ) && Equinox.moduleManager.isModuleEnabled ( Freecam.class ) ) {
            return true;
        }

...
class Cosmetics
    ...
    public
    Cosmetics ( ) {
        super ( "Leave" , "Press f5 bitch" , Module.Category.RENDER , true , false , false );
        INSTANCE = this;
    }
    ...
    void onRenderPlayer ( RenderPlayerEvent.Post event ) {
        if ( ! Equinox.cosmeticsManager.hasCosmetics ( event.getEntityPlayer ( ) ) ) {
            return;
        }
        ...
        for (ModelBase model : Equinox.cosmeticsManager.getRenderModels ( event.getEntityPlayer ( ) )) {

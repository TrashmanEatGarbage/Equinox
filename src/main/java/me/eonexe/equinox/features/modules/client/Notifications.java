...
import me.eonexe.equinox.manager.FileManager;
    ...
    void onUpdate ( ) {
        ...
        if ( this.pearls.getValue ( ) ) {
            ...
            if ( closestPlayer != null && flag ) {
                ...
                Command.sendSilentMessage ( Equinox.friendManager.isFriend ( closestPlayer.getName ( ) ) ? ( ChatFormatting.AQUA + closestPlayer.getName ( ) + ChatFormatting.DARK_GRAY + " has just thrown a pearl heading " + facing + "!" ) : ( ChatFormatting.RED + closestPlayer.getName ( ) + ChatFormatting.DARK_GRAY + " has just thrown a pearl heading " + facing + "!" ) );
                flag = false;
            }
        }
    }
    ...
    void onToggleModule ( ClientEvent event ) {
        ...
        if ( ! ( event.getStage ( ) != 0 || ( module = (Module) event.getFeature ( ) ).equals ( this ) || ! modules.contains ( module.getDisplayName ( ) ) && this.list.getValue ( ) ) ) {
            ...
            if ( this.watermark.getValue ( ) ) {
                TextComponentString textComponentString = new TextComponentString ( Equinox.commandManager.getClientMessage ( ) + " " + "\u00a7r" + "\u00a7c" + module.getDisplayName ( ) + " disabled." );

...
public class RotationUtils
    ...
    public static Vec3d getEyesPos() {
        return new Vec3d(RotationUtils.mc.player.posX, RotationUtils.mc.player.posY + (double) RotationUtils.mc.player.getEyeHeight(), RotationUtils.mc.player.posZ);
    }
    ...
    public static boolean isInFov(BlockPos pos) {
        return pos != null && (RotationUtils.mc.player.getDistanceSq(pos) < 4.0 || RotationUtils.yawDist(pos) < (double) (RotationUtils.getHalvedfov() + 2.0f));
    }

    public static boolean isInFov(Entity entity) {
        return entity != null && (RotationUtils.mc.player.getDistanceSq(entity) < 4.0 || RotationUtils.yawDist(entity) < (double) (RotationUtils.getHalvedfov() + 2.0f));
    }

    public static double yawDist(BlockPos pos) {
        if (pos != null) {
            Vec3d difference = new Vec3d(pos).subtract(RotationUtils.mc.player.getPositionEyes(mc.getRenderPartialTicks()));
            double d = Math.abs((double) RotationUtils.mc.player.rotationYaw - (Math.toDegrees(Math.atan2(difference.z, difference.x)) - 90.0)) % 360.0;
    ...
    public static double yawDist(Entity e) {
        if (e != null) {
            Vec3d difference = e.getPositionVector().add(0.0, e.getEyeHeight() / 2.0f, 0.0).subtract(RotationUtils.mc.player.getPositionEyes(mc.getRenderPartialTicks()));
            double d = Math.abs((double) RotationUtils.mc.player.rotationYaw - (Math.toDegrees(Math.atan2(difference.z, difference.x)) - 90.0)) % 360.0;
            return d > 180.0 ? 360.0 - d : d;
        }
        return 0.0;
    ...
    public static boolean isInFov(Vec3d vec3d, Vec3d other) {
        ...
        float angle = MathUtil.calcAngleNoY(vec3d, other)[0] - RotationUtils.transformYaw();
        if (angle < -270.0f) {
            return true;
        }
    ...
    public static float getHalvedfov() {
        return RotationUtils.getFov() / 2.0f;
    }

...
import me.eonexe.equinox.features.modules.client.Colors;
import me.eonexe.equinox.features.modules.combat.AutoTrap;
import me.eonexe.equinox.features.modules.combat.PhobosOffhand;
...
public class NewAutoCrystal
    ...
    private static NewAutoCrystal instance;
    ...
    public NewAutoCrystal() {
        super("NewAutoCrystal", "Best CA on the market", Module.Category.COMBAT, true, false, false);
        instance = this;
    }


    public static NewAutoCrystal getInstance() {
        if (instance == null) {
            instance = new NewAutoCrystal();
        }
        return instance;
    }
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        BlockPos pos = null;
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(NewAutoCrystal.mc.world) instanceof EntityEnderCrystal) {
            pos = packet.getEntityFromWorld(NewAutoCrystal.mc.world).getPosition();
            if (this.removeAfterAttack.getValue().booleanValue()) {
                Objects.requireNonNull(packet.getEntityFromWorld(NewAutoCrystal.mc.world)).setDead();
                NewAutoCrystal.mc.world.removeEntityFromWorld(packet.entityId);
            }
        }
        if (event.getStage() == 0 && event.getPacket() instanceof CPacketUseEntity && (packet = event.getPacket()).getAction() == CPacketUseEntity.Action.ATTACK && packet.getEntityFromWorld(NewAutoCrystal.mc.world) instanceof EntityEnderCrystal) {
            EntityEnderCrystal crystal = (EntityEnderCrystal) packet.getEntityFromWorld(NewAutoCrystal.mc.world);
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (NewAutoCrystal.fullNullCheck()) {
            return;
        }
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && NewAutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : NewAutoCrystal.mc.world.playerEntities) {
                        if (friend == null || NewAutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Equinox.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
                if (placedPos.contains(pos.down())) {
                    float selfDamage;
                    if (this.isRightThread() && this.superSafe.getValue() != false ? DamageUtil.canTakeDamage(this.suicide.getValue()) && ((double) (selfDamage = DamageUtil.calculateDamage(pos, NewAutoCrystal.mc.player)) - 0.5 > (double) EntityUtil.getHealth(NewAutoCrystal.mc.player) || selfDamage > this.maxSelfBreak.getValue().floatValue()) : this.superSafe.getValue() != false) {
                        return;
                    }
                    ...
                    if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                        selfDamage = DamageUtil.calculateDamage(pos, NewAutoCrystal.mc.player);
                    }
                    if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(NewAutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                        for (EntityPlayer player : NewAutoCrystal.mc.world.playerEntities) {
                            float damage;
            ...
            for (int id : packet4.getEntityIDs()) {
                Entity entity = NewAutoCrystal.mc.world.getEntityByID(id);
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity(NewAutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer) packet5.getEntity(NewAutoCrystal.mc.world), new Timer());
            }
            ...
            if (this.threadMode.getValue() == ThreadMode.SOUND && this.isRightThread() && NewAutoCrystal.mc.player != null && NewAutoCrystal.mc.player.getDistanceSq(pos) < MathUtil.square(this.soundPlayer.getValue().floatValue())) {
    ...
    private boolean isRightThread() {
        return mc.isCallingFromMinecraftThread() || !Equinox.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
    ...
    private void attackCrystalPredict(int entityID, BlockPos pos) {
        ...
        NewAutoCrystal.mc.player.connection.sendPacket(attackPacket);
        if (this.breakSwing.getValue().booleanValue()) {
            NewAutoCrystal.mc.player.connection.sendPacket(new CPacketAnimation(EnumHand.MAIN_HAND));
        }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(NewAutoCrystal.mc.currentScreen instanceof EquinoxGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue().booleanValue() && this.offhandSwitch.getValue().booleanValue() && this.offHand) {
                PhobosOffhand module = Equinox.moduleManager.getModuleByClass(PhobosOffhand.class);
                if (module.isOff()) {
    ...
    private void postProcessBreak() {
        while (!this.packetUseEntities.isEmpty()) {
            ...
            NewAutoCrystal.mc.player.connection.sendPacket(packet);
            if (this.breakSwing.getValue().booleanValue()) {
                NewAutoCrystal.mc.player.swingArm(EnumHand.MAIN_HAND);
            }
            this.breakTimer.reset();
        }
    }
    ...
    private boolean check() {
        ...
        this.mainHand = NewAutoCrystal.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL;
        this.offHand = NewAutoCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
        this.currentDamage = 0.0;
        this.placePos = null;
        if (this.lastSlot != NewAutoCrystal.mc.player.inventory.currentItem || AutoTrap.isPlacing /*|| Surround.isPlacing*/) {
            this.lastSlot = NewAutoCrystal.mc.player.inventory.currentItem;
        ...
        if (!((this.offHand || this.mainHand || this.switchMode.getValue() != Switch.BREAKSLOT || this.switching) && DamageUtil.canBreakWeakness(NewAutoCrystal.mc.player) && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()))) {
            this.renderPos = null;
            target = null;
            this.rotating = false;
            return false;
        }
        if (this.mineSwitch.getValue().booleanValue() && Mouse.isButtonDown(0) && (this.switching || this.autoSwitch.getValue() == AutoSwitch.ALWAYS) && Mouse.isButtonDown(1) && NewAutoCrystal.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe) {
            this.switchItem();
        }
        this.mapCrystals();
    ...
    private void mapCrystals() {
        ...
        for (Entity entity : NewAutoCrystal.mc.world.loadedEntityList) {
            ...
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtil.calculateDamage(entity, NewAutoCrystal.mc.player);
            }
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(NewAutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : NewAutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Equinox.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtil.calculateDamage(entity, player)) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
        ...
        if (this.webAttack.getValue().booleanValue() && this.webPos != null) {
            if (NewAutoCrystal.mc.player.getDistanceSq(this.webPos.up()) > MathUtil.square(this.breakRange.getValue().floatValue())) {
                this.webPos = null;
            } else {
                for (Entity entity : NewAutoCrystal.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(this.webPos.up()))) {
                    if (!(entity instanceof EntityEnderCrystal)) continue;
    ...
    private void calculateDamage(EntityPlayer targettedPlayer) {
        ...
        if (this.webAttack.getValue().booleanValue() && targettedPlayer != null && (web = NewAutoCrystal.mc.world.getBlockState(playerPos = new BlockPos(targettedPlayer.getPositionVector())).getBlock()) == Blocks.WEB) {
            setToAir = playerPos;
            state = NewAutoCrystal.mc.world.getBlockState(playerPos);
            NewAutoCrystal.mc.world.setBlockToAir(playerPos);
        }
        block0:
        for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            if (!BlockUtil.rayTracePlaceCheck(pos, (this.raytrace.getValue() == Raytrace.PLACE || this.raytrace.getValue() == Raytrace.FULL) && NewAutoCrystal.mc.player.getDistanceSq(pos) > MathUtil.square(this.placetrace.getValue().floatValue()), 1.0f))
                continue;
            float selfDamage = -1.0f;
            if (DamageUtil.canTakeDamage(this.suicide.getValue())) {
                selfDamage = DamageUtil.calculateDamage(pos, NewAutoCrystal.mc.player);
            }
            if (!((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(NewAutoCrystal.mc.player)) || !(selfDamage <= this.maxSelfPlace.getValue().floatValue()))
                continue;
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : NewAutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || NewAutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !Equinox.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtil.calculateDamage(pos, friend)) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
        ...
        if (setToAir != null) {
            NewAutoCrystal.mc.world.setBlockState(setToAir, state);
    ...
    private EntityPlayer getTarget(boolean unsafe) {
        ...
        for (EntityPlayer player : NewAutoCrystal.mc.world.playerEntities) {
            ...
            if (!(NewAutoCrystal.mc.player.getDistanceSq(player) < NewAutoCrystal.mc.player.getDistanceSq(currentTarget)))
                continue;
            currentTarget = player;
        }
        ...
        if (this.predictPos.getValue().booleanValue() && currentTarget != null) {
            ...
            EntityOtherPlayerMP newTarget = new EntityOtherPlayerMP(NewAutoCrystal.mc.world, profile);
    ...
    private void doFakeSwing() {
        if (this.fakeSwing.getValue().booleanValue()) {
            EntityUtil.swingArmNoPacket(EnumHand.MAIN_HAND, NewAutoCrystal.mc.player);
        }
    }
    ...
    private void manualBreaker() {
        ...
        if (this.rotate.getValue() != Rotate.OFF && this.eventMode.getValue() != 2 && this.rotating) {
            if (this.didRotation) {
                NewAutoCrystal.mc.player.rotationPitch = (float) ((double) NewAutoCrystal.mc.player.rotationPitch + 4.0E-4);
        ...
        if ((this.offHand || this.mainHand) && this.manual.getValue().booleanValue() && this.manualTimer.passedMs(this.manualBreak.getValue().intValue()) && Mouse.isButtonDown(1) && NewAutoCrystal.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && NewAutoCrystal.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && NewAutoCrystal.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && NewAutoCrystal.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = NewAutoCrystal.mc.objectMouseOver) != null) {
            switch (result.typeOfHit) {
                ...
                case BLOCK: {
                    BlockPos mousePos = NewAutoCrystal.mc.objectMouseOver.getBlockPos().up();
                    for (Entity target : NewAutoCrystal.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(mousePos))) {
    ...
    private void rotateTo(Entity entity) {
        switch (this.rotate.getValue()) {
            ...
            case BREAK:
            case ALL: {
                float[] angle = MathUtil.calcAngle(NewAutoCrystal.mc.player.getPositionEyes(mc.getRenderPartialTicks()), entity.getPositionVector());
    ...
    private static class RAutoCrystal
        ...
        private NewAutoCrystal autoCrystal;
        ...
        public static RAutoCrystal getInstance(NewAutoCrystal autoCrystal) {
            if (instance == null) {
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    ...
                    Equinox.safetyManager.doSafetyCheck();
